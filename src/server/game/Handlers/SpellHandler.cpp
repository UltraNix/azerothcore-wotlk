...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
    ...
    sLog->outChinaTown( "WORLD: CMSG_USE_ITEM packet, bagIndex: %u, slot: %u, castCount: %u, spellId: %u, Item: %u, glyphIndex: %u, data length = %i", bagIndex, slot, castCount, spellId, pItem->GetEntry(), glyphIndex, (uint32)recvPacket.size());
...
{
    sLog->outChinaTown( "WORLD: CMSG_OPEN_ITEM packet, data length = %i", (uint32)recvPacket.size());
...
void WorldSession::HandleGameObjectUseOpcode(WorldPacket & recvData)
{
    ...
    sLog->outChinaTown( "WORLD: Recvd CMSG_GAMEOBJ_USE Message [guid=%u]", GUID_LOPART(guid));
...
void WorldSession::HandleGameobjectReportUse(WorldPacket& recvPacket)
{
    ...
    sLog->outChinaTown( "WORLD: Recvd CMSG_GAMEOBJ_REPORT_USE Message [in game guid: %u]", GUID_LOPART(guid));
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    sLog->outChinaTown( "WORLD: got cast spell packet, castCount: %u, spellId: %u, castFlags: %u, data length = %u", castCount, spellId, castFlags, (uint32)recvPacket.size());
...
{
    sLog->outChinaTown( "WORLD: CMSG_SELF_RES");                  // empty opcode
...
{
    sLog->outChinaTown( "WORLD: CMSG_GET_MIRRORIMAGE_DATA");
    uint64 guid;
    recvData >> guid;
...
{
    sLog->outChinaTown( "WORLD: CMSG_UPDATE_PROJECTILE_POSITION");
