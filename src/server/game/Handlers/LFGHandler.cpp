...
void WorldSession::HandleLfgJoinOpcode(WorldPacket& recvData)
{
    ...
    if (!numDungeons)
    {
        sLog->outChinaTown( "CMSG_LFG_JOIN [" UI64FMTD "] no dungeons selected", GetPlayer()->GetGUID());
        recvData.rfinish();
        return;
    }
    ...
    std::string comment;
    recvData >> comment;
    sLog->outChinaTown( "CMSG_LFG_JOIN [" UI64FMTD "] roles: %u, Dungeons: %u, Comment: %s", GetPlayer()->GetGUID(), roles, uint8(newDungeons.size()), comment.c_str());
...
void WorldSession::HandleLfgLeaveOpcode(WorldPacket&  /*recvData*/)
{
    ...
    sLog->outChinaTown( "CMSG_LFG_LEAVE [" UI64FMTD "] in group: %u", guid, group ? 1 : 0);
...
void WorldSession::HandleLfgProposalResultOpcode(WorldPacket& recvData)
{
    ...
    //sLog->outChinaTown( "CMSG_LFG_PROPOSAL_RESULT [" UI64FMTD "] proposal: %u accept: %u", GetPlayer()->GetGUID(), lfgGroupID, accept ? 1 : 0);
...
void WorldSession::HandleLfgSetRolesOpcode(WorldPacket& recvData)
{
    ...
    if (!group)
    {
        sLog->outChinaTown( "CMSG_LFG_SET_ROLES [" UI64FMTD "] Not in group", guid);
        return;
    }
    uint64 gguid = group->GetGUID();
    sLog->outChinaTown( "CMSG_LFG_SET_ROLES: Group [" UI64FMTD "], Player [" UI64FMTD "], Roles: %u", gguid, guid, roles);
...
void WorldSession::HandleLfgSetCommentOpcode(WorldPacket&  recvData)
{
    std::string comment;
    recvData >> comment;
  //  sLog->outChinaTown( "CMSG_LFG_SET_COMMENT [" UI64FMTD "] comment: %s", guid, comment.c_str());
...
void WorldSession::HandleLfgSetBootVoteOpcode(WorldPacket& recvData)
{
    ...
    uint64 guid = GetPlayer()->GetGUID();
    sLog->outChinaTown( "CMSG_LFG_SET_BOOT_VOTE [" UI64FMTD "] agree: %u", guid, agree ? 1 : 0);
...
void WorldSession::HandleLfgTeleportOpcode(WorldPacket& recvData)
{
    ...
    sLog->outChinaTown( "CMSG_LFG_TELEPORT [" UI64FMTD "] out: %u", GetPlayer()->GetGUID(), out ? 1 : 0);
...
{
    uint64 guid = GetPlayer()->GetGUID();
    sLog->outChinaTown( "CMSG_LFG_PLAYER_LOCK_INFO_REQUEST [" UI64FMTD "]", guid);
    ...
    sLog->outChinaTown( "SMSG_LFG_PLAYER_INFO [" UI64FMTD "]", guid);
...
{
    uint64 guid = GetPlayer()->GetGUID();
    sLog->outChinaTown( "CMSG_LFG_PARTY_LOCK_INFO_REQUEST [" UI64FMTD "]", guid);
    ...
    sLog->outChinaTown( "SMSG_LFG_PARTY_INFO [" UI64FMTD "]", guid);
...
{
    sLog->outChinaTown( "SMSG_LFG_ROLE_CHOSEN [" UI64FMTD "] guid: [" UI64FMTD "] roles: %u", GetPlayer()->GetGUID(), guid, roles);
...
void WorldSession::SendLfgRoleCheckUpdate(lfg::LfgRoleCheck const& roleCheck)
{
    ...
    sLog->outChinaTown( "SMSG_LFG_ROLE_CHECK_UPDATE [" UI64FMTD "]", GetPlayer()->GetGUID());
...
void WorldSession::SendLfgJoinResult(lfg::LfgJoinResultData const& joinData)
{
    ...
    sLog->outChinaTown( "SMSG_LFG_JOIN_RESULT [" UI64FMTD "] checkResult: %u checkValue: %u", GetPlayer()->GetGUID(), joinData.result, joinData.state);
...
{
    sLog->outChinaTown( "SMSG_LFG_QUEUE_STATUS [" UI64FMTD "] dungeon: %u - waitTime: %d - avgWaitTime: %d - waitTimeTanks: %d - waitTimeHealer: %d - waitTimeDps: %d - queuedTime: %u - tanks: %u - healers: %u - dps: %u",
...
void WorldSession::SendLfgBootProposalUpdate(lfg::LfgPlayerBoot const& boot)
{
    ...
    sLog->outChinaTown( "SMSG_LFG_BOOT_PROPOSAL_UPDATE [" UI64FMTD "] inProgress: %u - didVote: %u - agree: %u - victim: [" UI64FMTD "] votes: %u - agrees: %u - left: %u - needed: %u - reason %s",
...
void WorldSession::SendLfgUpdateProposal(lfg::LfgProposal const& proposal)
{
    ...
    sLog->outChinaTown( "SMSG_LFG_PROPOSAL_UPDATE [" UI64FMTD "] state: %u", guid, proposal.state);
...
{
    sLog->outChinaTown( "SMSG_LFG_LFR_LIST [" UI64FMTD "] update: %u", GetPlayer()->GetGUID(), update ? 1 : 0);
...
{
    sLog->outChinaTown( "SMSG_LFG_DISABLED [" UI64FMTD "]", GetPlayer()->GetGUID());
...
{
    sLog->outChinaTown( "SMSG_LFG_OFFER_CONTINUE [" UI64FMTD "] dungeon entry: %u", GetPlayer()->GetGUID(), dungeonEntry);
...
{
    sLog->outChinaTown( "SMSG_LFG_TELEPORT_DENIED [" UI64FMTD "] reason: %u", GetPlayer()->GetGUID(), err);
