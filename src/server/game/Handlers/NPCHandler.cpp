...
void WorldSession::HandleTabardVendorActivateOpcode(WorldPacket & recvData)
{
    ...
    if (!unit)
    {
        sLog->outChinaTown( "WORLD: HandleTabardVendorActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::HandleBankerActivateOpcode(WorldPacket & recvData)
{
    ...
    sLog->outChinaTown( "WORLD: Received CMSG_BANKER_ACTIVATE");
    ...
    if (!unit)
    {
        sLog->outChinaTown( "WORLD: HandleBankerActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
{
    sLog->outChinaTown( "WORLD: SendTrainerList");

    Creature* unit = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_TRAINER);
    if (!unit)
    {
        sLog->outChinaTown( "WORLD: SendTrainerList - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    if (!ci)
    {
        sLog->outChinaTown( "WORLD: SendTrainerList - (GUID: %u) NO CREATUREINFO!", GUID_LOPART(guid));
        return;
    }
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket & recvData)
{
    ...
    sLog->outChinaTown( "WORLD: Received CMSG_TRAINER_BUY_SPELL NpcGUID=%u, learn spell id is: %u", uint32(GUID_LOPART(guid)), spellId);

    Creature* unit = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_TRAINER);
    if (!unit)
    {
        sLog->outChinaTown( "WORLD: HandleTrainerBuySpellOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
{
    sLog->outChinaTown( "WORLD: Received CMSG_GOSSIP_HELLO");
    ...
    if (!unit)
    {
        sLog->outChinaTown( "WORLD: HandleGossipHelloOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
{
    sLog->outChinaTown( "WORLD: CMSG_SPIRIT_HEALER_ACTIVATE");
    ...
    if (!unit)
    {
        sLog->outChinaTown( "WORLD: HandleSpiritHealerActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::HandleBinderActivateOpcode(WorldPacket & recvData)
{
    ...
    if (!unit)
    {
        sLog->outChinaTown( "WORLD: HandleBinderActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(npcGUID)));
        return;
    }
...
{
    sLog->outChinaTown( "WORLD: Recv MSG_LIST_STABLED_PETS");
    uint64 npcGUID;
...
void WorldSession::SendStablePetCallback(PreparedQueryResult result, uint64 guid)
{
    ...
    sLog->outChinaTown( "WORLD: Recv MSG_LIST_STABLED_PETS Send.");
...
{
    sLog->outChinaTown( "WORLD: Recv CMSG_STABLE_PET");
    uint64 npcGUID;
...
{
    sLog->outChinaTown( "WORLD: Recv CMSG_UNSTABLE_PET.");
    uint64 npcGUID;
    uint32 petnumber;
...
{
    sLog->outChinaTown( "WORLD: Recv CMSG_BUY_STABLE_SLOT.");
    uint64 npcGUID;
...
{
    sLog->outChinaTown( "HandleStableRevivePet: Not implemented");
}

void WorldSession::HandleStableSwapPet(WorldPacket & recvData)
{
    sLog->outChinaTown( "WORLD: Recv CMSG_STABLE_SWAP_PET.");
    uint64 npcGUID;
    uint32 petId;
...
{
    sLog->outChinaTown( "WORLD: CMSG_REPAIR_ITEM");
    ...
    if (!unit)
    {
        sLog->outChinaTown( "WORLD: HandleRepairItemOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(npcGUID)));
        return;
    }
    ...
    if (itemGUID)
    {
        sLog->outChinaTown( "ITEM: Repair item, itemGUID = %u, npcGUID = %u", GUID_LOPART(itemGUID), GUID_LOPART(npcGUID));
    ...
    {
        sLog->outChinaTown( "ITEM: Repair all items, npcGUID = %u", GUID_LOPART(npcGUID));
