...
void WorldSession::HandleItemQuerySingleOpcode(WorldPacket & recvData)
{
    ...
    {
       sLog->outChinaTown( "WORLD: CMSG_ITEM_QUERY_SINGLE - NO item INFO! (ENTRY: %u)", item);
...
{
   sLog->outChinaTown( "WORLD: Received CMSG_SELL_ITEM");
    ...
    if (!creature)
    {
       sLog->outChinaTown( "WORLD: HandleSellItemOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorguid)));
...
{
   sLog->outChinaTown( "WORLD: Received CMSG_BUYBACK_ITEM");
    uint64 vendorguid;
    uint32 slot;
    ...
    if (!creature)
    {
       sLog->outChinaTown( "WORLD: HandleBuybackItem - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorguid)));
...
{
   sLog->outChinaTown( "WORLD: Received CMSG_BUY_ITEM_IN_SLOT");
...
{
   sLog->outChinaTown( "WORLD: Received CMSG_BUY_ITEM");
    uint64 vendorguid;
    uint32 item, slot, count;
    uint8 unk1;
...
void WorldSession::HandleListInventoryOpcode(WorldPacket & recvData)
{
   ...
   sLog->outChinaTown( "WORLD: Recvd CMSG_LIST_INVENTORY");
...
{
   sLog->outChinaTown( "WORLD: Sent SMSG_LIST_INVENTORY");

    Creature* vendor = GetPlayer()->GetNPCIfCanInteractWith(vendorGuid, UNIT_NPC_FLAG_VENDOR);
    if (!vendor)
    {
       sLog->outChinaTown( "WORLD: SendListInventory - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorGuid)));
        _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, NULL, 0, 0);
        return;
    }
...
{
   sLog->outChinaTown( "WORLD: CMSG_BUY_BANK_SLOT");
...
{
   sLog->outChinaTown( "WORLD: CMSG_AUTOBANK_ITEM");
    uint8 srcbag, srcslot;

    recvPacket >> srcbag >> srcslot;
   sLog->outChinaTown( "STORAGE: receive srcbag = %u, srcslot = %u", srcbag, srcslot);
...
{
   sLog->outChinaTown( "WORLD: CMSG_AUTOSTORE_BANK_ITEM");
    uint8 srcbag, srcslot;
...
void WorldSession::HandleSetAmmoOpcode(WorldPacket & recvData)
{
   ...
   sLog->outChinaTown( "WORLD: CMSG_SET_AMMO");
    uint32 item;
...
void WorldSession::HandleItemNameQueryOpcode(WorldPacket & recvData)
{
   ...
   sLog->outChinaTown( "WORLD: CMSG_ITEM_NAME_QUERY %u", itemid);
...
{
   sLog->outChinaTown( "Received opcode CMSG_WRAP_ITEM");
   ...
   sLog->outChinaTown( "WRAP: receive gift_bag = %u, gift_slot = %u, item_bag = %u, item_slot = %u", gift_bag, gift_slot, item_bag, item_slot);
...
{
   sLog->outChinaTown( "WORLD: CMSG_SOCKET_GEMS");
...
{
   sLog->outChinaTown( "WORLD: CMSG_CANCEL_TEMP_ENCHANTMENT");
...
{
   sLog->outChinaTown( "WORLD: CMSG_ITEM_REFUND_INFO");
    ...
    if (!item)
    {
       sLog->outChinaTown( "Item refund: item not found!");
        return;
    }
...
{
   sLog->outChinaTown( "WORLD: CMSG_ITEM_REFUND");
    uint64 guid;
    recvData >> guid;                                      // item guid
...
void WorldSession::HandleItemTextQuery(WorldPacket & recvData )
{
   ...
   sLog->outChinaTown( "CMSG_ITEM_TEXT_QUERY item guid: %u", GUID_LOPART(itemGuid));
