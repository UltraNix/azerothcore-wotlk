...
{
    sLog->outChinaTown( "WORLD: Recvd CMSG_REPOP_REQUEST Message");
    ...
    {
        sLog->outChinaTown( "HandleRepopRequestOpcode: got request after player %s(%d) was killed and before he was updated", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow());
...
{
    sLog->outChinaTown( "WORLD: CMSG_GOSSIP_SELECT_OPTION");
    ...
    if (IS_CRE_OR_VEH_GUID(guid))
    {
        ...
        if (!unit)
        {
            sLog->outChinaTown( "WORLD: HandleGossipSelectOptionOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else if (IS_GAMEOBJECT_GUID(guid))
    {
        ...
        if (!go)
        {
            sLog->outChinaTown( "WORLD: HandleGossipSelectOptionOpcode - GameObject (GUID: %u) not found.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else
    {
        sLog->outChinaTown( "WORLD: HandleGossipSelectOptionOpcode - unsupported GUID type for highguid %u. lowpart %u.", uint32(GUID_HIPART(guid)), uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    {
        sLog->outChinaTown( "WORLD: HandleGossipSelectOptionOpcode - Script reloaded while in use, ignoring and set new scipt id");
        if (unit)
...
void WorldSession::HandleWhoOpcode(WorldPacket & recv_data)
{
    ...
    for (uint32 i = 0; i < zones_count; ++i)
    {
        ...
        zoneids[i] = temp;
        sLog->outChinaTown( "Zone %u: %u", i, zoneids[i]);
    }
    ...
    sLog->outChinaTown( "Minlvl %u, maxlvl %u, name %s, guild %s, racemask %u, classmask %u, zones %u, strings %u", level_min, level_max, player_name.c_str(), guild_name.c_str(), racemask, classmask, zones_count, str_count);
    ...
    for (uint32 i = 0; i < str_count; ++i)
    {
        ...
        sLog->outChinaTown( "String %u: %s", i, temp.c_str());
    }
    ...
    SendPacket(&data);
    sLog->outChinaTown( "WORLD: Send SMSG_WHO Message");
}

void WorldSession::HandleLogoutRequestOpcode(WorldPacket & /*recv_data*/)
{
    sLog->outChinaTown( "WORLD: Recvd CMSG_LOGOUT_REQUEST Message, security - %u", GetSecurity());
...
{
    sLog->outChinaTown( "WORLD: Recvd CMSG_PLAYER_LOGOUT Message");
}
...
void WorldSession::HandleZoneUpdateOpcode(WorldPacket & recv_data)
{
    ...
    sLog->outChinaTown( "WORLD: Recvd ZONE_UPDATE: %u", newZone);
...
void WorldSession::HandleContactListOpcode(WorldPacket & recv_data)
{
    uint32 unk;
    recv_data >> unk;
    sLog->outChinaTown( "WORLD: Received CMSG_CONTACT_LIST - Unk: %d", unk);
    _player->GetSocial()->SendSocialList(_player);
}

void WorldSession::HandleAddFriendOpcode(WorldPacket & recv_data)
{
    sLog->outChinaTown( "WORLD: Received CMSG_ADD_FRIEND");
    ...
    sLog->outChinaTown( "WORLD: %s asked to add friend : '%s'", GetPlayer()->GetName().c_str(), friendName.c_str());
        ...
        if (friendGuid)
        {
            ...
            else
            {
                ...
                if (!GetPlayer()->GetSocial()->AddToSocialList(guidLow, false))
                {
                    ...
                    sLog->outChinaTown( "WORLD: %s's friend list is full.", GetPlayer()->GetName().c_str());
                }
            }
    ...
    sLog->outChinaTown( "WORLD: Sent (SMSG_FRIEND_STATUS)");
}
...
void WorldSession::HandleDelFriendOpcode(WorldPacket & recv_data)
{
    ...
    sLog->outChinaTown( "WORLD: Received CMSG_DEL_FRIEND");
    ...
    sLog->outChinaTown( "WORLD: Sent motd (SMSG_FRIEND_STATUS)");
}

void WorldSession::HandleAddIgnoreOpcode(WorldPacket & recv_data)
{
    sLog->outChinaTown( "WORLD: Received CMSG_ADD_IGNORE");
    ...
    sLog->outChinaTown( "WORLD: %s asked to Ignore: '%s'",
...
void WorldSession::HandleDelIgnoreOpcode(WorldPacket & recv_data)
{
    ...
    sLog->outChinaTown( "WORLD: Received CMSG_DEL_IGNORE");
...
{
    sLog->outChinaTown( "CMSG_SET_CONTACT_NOTES");
    uint64 guid;
    std::string note;
    recv_data >> guid >> note;
...
void WorldSession::HandleBugOpcode(WorldPacket & recv_data)
{
    ...
    if (suggestion == 0)
        sLog->outChinaTown( "WORLD: Received CMSG_BUG [Bug Report]");
    else
        sLog->outChinaTown( "WORLD: Received CMSG_BUG [Suggestion]");

    sLog->outChinaTown( "%s", type.c_str());
    sLog->outChinaTown( "%s", content.c_str());
...
{
    sLog->outChinaTown( "WORLD: Received CMSG_RECLAIM_CORPSE");
...
{
    sLog->outChinaTown( "WORLD: Received CMSG_RESURRECT_RESPONSE");
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recv_data)
{
    ...
    sLog->outChinaTown( "CMSG_AREATRIGGER. Trigger ID: %u", triggerId);

    Player* player = GetPlayer();
    if (player->IsInFlight())
    {
        sLog->outChinaTown( "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) in flight, ignore Area Trigger ID:%u",
    ...
    if (!atEntry)
    {
        sLog->outChinaTown( "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) send unknown (by DBC) Area Trigger ID:%u",
            player->GetName().c_str(), player->GetGUIDLow(), triggerId);
        return;
    }
    ...
    {
        sLog->outChinaTown( "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (trigger map: %u player map: %u), ignore Area Trigger ID: %u",
...
{
    sLog->outChinaTown( "WORLD: Received CMSG_UPDATE_ACCOUNT_DATA");

    uint32 type, timestamp, decompressedSize;
    recv_data >> type >> timestamp >> decompressedSize;

    sLog->outChinaTown( "UAD: type %u, time %u, decompressedSize %u", type, timestamp, decompressedSize);
...
{
    sLog->outChinaTown( "WORLD: Received CMSG_REQUEST_ACCOUNT_DATA");

    uint32 type;
    recv_data >> type;

    sLog->outChinaTown( "RAD: type %u", type);
    ...
    {
        sLog->outChinaTown( "RAD: Failed to compress account data");
        return;
    }
...
{
    sLog->outChinaTown( "WORLD: Received CMSG_SET_ACTION_BUTTON");
    uint8 button;
    uint32 packetData;
...
{
    sLog->outChinaTown( "WORLD: Received CMSG_COMPLETE_CINEMATIC");
}

void WorldSession::HandleNextCinematicCamera(WorldPacket & /*recv_data*/)
{
    sLog->outChinaTown( "WORLD: Received CMSG_NEXT_CINEMATIC_CAMERA");
}
...
{
    sLog->outChinaTown( "WORLD: CMSG_MOVE_FEATHER_FALL_ACK");
...
void WorldSession::HandleInspectOpcode(WorldPacket& recv_data)
{
    ...
    sLog->outChinaTown( "WORLD: Received CMSG_INSPECT");

    Player* player = ObjectAccessor::GetPlayer(*_player, guid);
    if (!player)
    {
        sLog->outChinaTown( "CMSG_INSPECT: No player found from GUID: " UI64FMTD, guid);
        return;
    }
...
void WorldSession::HandleInspectHonorStatsOpcode(WorldPacket& recv_data)
{
    ...
    if (!player)
    {
        sLog->outChinaTown( "MSG_INSPECT_HONOR_STATS: No player found from GUID: " UI64FMTD, guid);
        return;
    }
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recv_data)
{
    ...
    sLog->outChinaTown( "WORLD: Received CMSG_WORLD_TELEPORT");

    if (GetPlayer()->IsInFlight())
    {
        sLog->outChinaTown( "Player '%s' (GUID: %u) in flight, ignore worldport command.", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow());
        return;
    }

    sLog->outChinaTown( "CMSG_WORLD_TELEPORT: Player = %s, Time = %u, map = %u, x = %f, y = %f, z = %f, o = %f", GetPlayer()->GetName().c_str(), time, mapid, PositionX, PositionY, PositionZ, Orientation);
...
{
    sLog->outChinaTown( "Received opcode CMSG_WHOIS");
    std::string charname;
    recv_data >> charname;
    ...
    sLog->outChinaTown( "Received whois command from player %s for character %s", GetPlayer()->GetName().c_str(), charname.c_str());
}

void WorldSession::HandleComplainOpcode(WorldPacket & recv_data)
{
    sLog->outChinaTown( "WORLD: CMSG_COMPLAIN");
    ...
    sLog->outChinaTown( "REPORT SPAM: type %u, guid %u, unk1 %u, unk2 %u, unk3 %u, unk4 %u, message %s", spam_type, GUID_LOPART(spammer_guid), unk1, unk2, unk3, unk4, description.c_str());
}

void WorldSession::HandleRealmSplitOpcode(WorldPacket & recv_data)
{
    sLog->outChinaTown( "CMSG_REALM_SPLIT");
...
{
    sLog->outChinaTown( "WORLD: CMSG_FAR_SIGHT");
...
{
    sLog->outChinaTown( "CMSG_SET_TITLE");
...
{
    sLog->outChinaTown( "WORLD: CMSG_RESET_INSTANCES");
...
{
    sLog->outChinaTown( "MSG_SET_DUNGEON_DIFFICULTY");
...
{
    sLog->outChinaTown( "MSG_SET_RAID_DIFFICULTY");
...
{
    sLog->outChinaTown( "WORLD: CMSG_CANCEL_MOUNT_AURA");
...
{
    // fly mode on/off
    sLog->outChinaTown( "WORLD: CMSG_MOVE_SET_CAN_FLY_ACK");
...
void WorldSession::HandleSetTaxiBenchmarkOpcode(WorldPacket & recv_data)
{
    ...
    sLog->outChinaTown( "Client used \"/timetest %d\" command", mode);
}
...
{
    // empty opcode
    sLog->outChinaTown( "WORLD: CMSG_WORLD_STATE_UI_TIMER_UPDATE");
...
{
    // empty opcode
    sLog->outChinaTown( "WORLD: CMSG_READY_FOR_ACCOUNT_DATA_TIMES");
...
{
    sLog->outChinaTown( "WORLD: CMSG_AREA_SPIRIT_HEALER_QUERY");
...
{
    sLog->outChinaTown( "WORLD: CMSG_AREA_SPIRIT_HEALER_QUEUE");
...
{
    sLog->outChinaTown( "WORLD: CMSG_UPDATE_MISSILE_TRAJECTORY");
