...
void GameObject::Update(uint32 diff)
{
    ...
    switch (m_lootState)
    {
        ...
        case GO_JUST_DEACTIVATED:
        {
            ...
            if (sWorld->getBoolConfig(CONFIG_DECREASED_SPAWN_RATES) && GetGOInfo()->IsGameObjectForQuests() && !GetMap()->Instanceable() && !GetMap()->IsBattlegroundOrArena() && !hasValueableLoot)
            {
