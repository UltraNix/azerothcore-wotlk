            ...
            case SPELLFAMILY_GENERIC:
            {
                switch (auraId)
                {
                    ...
                    case 27808:
                        if (caster)
                        {
                            caster->CastCustomSpell(29879, SPELLVALUE_BASE_POINT0, int32(target->CountPctFromMaxHealth(26)), target, true, NULL, this);
                            if (GetTickNumber() == 1)
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (target->HasUnitState(UNIT_STATE_ISOLATED) || target->IsImmunedToDamage(GetSpellInfo()) || target->IsTotem())
    {
    ...
    if (caster && !target->IsAlive()) { //Seed of Corruption fix
        ...
        {
            caster->CastSpell(target, 47834, true);
            break;
        }
        case 47835: //Seed of Corruption rank 2
        {
            caster->CastSpell(target, 47833, true);
            break;
        }
