...
// Custom Spell Delay
uint64 GetCustomSpellDelay(SpellInfo const *spellEntry)
{
    // Warrior ---------------------------------------------------------------
    if (spellEntry->SpellFamilyFlags[1] & 0x1000000 && spellEntry->SpellFamilyName == SPELLFAMILY_WARRIOR)
        return 200;
...
void Spell::AddUnitTarget(Unit* target, uint32 effectMask, bool checkIfValid /*= true*/, bool implicit /*= true*/)
{
    ...
    m_delayMoment = GetCustomSpellDelay(m_spellInfo) ? GetCustomSpellDelay(m_spellInfo) : targetInfo.timeDelay;
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    else if (m_damage > 0)
    {
        ...
        if (canEffectTrigger)
        {
            ...
            caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx, damageInfo.damage, m_attackType, m_spellInfo, m_triggeredByAuraSpell, preMitigationDamage);
...
void Spell::_cast(bool skipCheck)
{
    ...
    SetExecutedCurrently(false);
    if (m_casttime && !m_spellInfo->HasAttribute(SPELL_ATTR2_NOT_RESET_AUTO_ACTIONS))
    {
        ...
        case 66663:
            if (!sWorld->PatchNotes(PATCH_320))
            {
                if (m_caster->ToPlayer())
                    ChatHandler(m_caster->ToPlayer()->GetSession()).PSendSysMessage("You can't use this spell before 3.2 content!");
                return SPELL_FAILED_DONT_REPORT;
            }
            break;
            // Ring of Valor
