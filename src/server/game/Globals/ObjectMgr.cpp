...
void ObjectMgr::LoadCreatureLocales()
{
    ...
    sLog->outString(">> Loaded %lu creature locale strings in %u ms", (uint32)_creatureLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void ObjectMgr::LoadGameObjectLocales()
{
    ...
    sLog->outString(">> Loaded %lu gameobject locale strings in %u ms", (uint32)_gameObjectLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void ObjectMgr::RequestItemDestroy( Item * item )
{
    if (item->IsBag())
    {
        ...
        {
            sLog->outBagCrash("Bag %p (GUID: %d, LowGuid: %d, Entry: %d, Slot: %d, BagSlot: %d) is requested to be destroyed",
                item, item->GetGUID(), item->GetGUIDLow(), item->GetEntry(), item->GetSlot(), item->GetBagSlot());
        }
    }
