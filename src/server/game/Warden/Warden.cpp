...
std::string Warden::Penalty(WardenCheck* check /*= NULL*/, uint16 checkFailed /*= 0*/)
{
    ...
    if (checkFailed)
        switch (checkFailed)
        {
            ...
            case 779: banReason += " (rEdoX Packet Editor)"; longBan = true; break;
            ...
            case 785: banReason += " (Hitchhikers Hack)"; longBan = true; break;
            case 786: banReason += " (WPE PRO)"; longBan = true; break;
            case 787: banReason += " (ICanHazSpeed)"; longBan = true; break;
            case 788: banReason += " (Maelstrom)"; longBan = true; break;
            case 789: banReason += " (Maelstrom)"; longBan = true; break;
        ...
        case WARDEN_ACTION_BAN:
            {
                ...
                sLog->outCheat("[Warden] Player: %s (Account: %u) has been banned for %s due to check failed: %u", _session->GetPlayerName().c_str(), _session->GetAccountId(), (longBan ? longDuration.c_str() : duration.str().c_str()), checkFailed);
