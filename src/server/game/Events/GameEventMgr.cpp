...
void GameEventMgr::SetHolidayEventTime(GameEventData& event)
{
    ...
    {
        sLog->outErrorDb("sql.sql", "Missing date or duration for holiday %u.", event.holiday_id);
        return;
    }
    ...
    sLog->outErrorDb("No suitable start date found for holiday %u.", event.holiday_id);
}
