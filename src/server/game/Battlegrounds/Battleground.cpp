...
void Battleground::EndBattleground(TeamId winnerTeamId)
{
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (isBattleground() && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_STORE_STATISTICS_ENABLE))
        {
            ...
            stmt->setBool(2, itr->second->GetTeam() == GetWinner());
        ...
        if (bgTeamId == winnerTeamId)
        {
            ...
            if (!isArena() && (GetBgTypeID() == BATTLEGROUND_AV || GetBgTypeID() == BATTLEGROUND_IC) && GetPlayersCountByTeam(TEAM_ALLIANCE) >= GetMinPlayersPerTeam() && GetPlayersCountByTeam(TEAM_HORDE) >= GetMinPlayersPerTeam())
                player->GiveXP(0.05 * player->GetUInt32Value(PLAYER_NEXT_LEVEL_XP), nullptr, 1.0f, true);
            ...
            if (!isArena())
            {
                if (player->GetQuestStatus(30005) == QUEST_STATUS_INCOMPLETE)
                    player->KilledMonsterCredit(200000, 0);
            }
...
void Battleground::AddPlayer(Player* player)
{
    ...
    player->m_crossfactionState = CrossFactionPlayerState::UpdatePlayerData;
}
