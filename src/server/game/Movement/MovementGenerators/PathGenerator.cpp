...
bool PathGenerator::CalculatePath( float destX, float destY, float destZ, bool forceDest )
{
    ...
    const Map* base = m_context->GetBaseMap();
...
void PathGenerator::BuildPolyPath( G3D::Vector3 const& startPos, G3D::Vector3 const& endPos, ACE_RW_Thread_Mutex& mmapLock )
{
...

    {
        ...
        if ( farFromStartPoly )
        {
            ...
            if ( sourceCanSwim )
            {
                if ( LIQUID_MAP_NO_WATER == baseMap->getLiquidStatus( startPos.x, startPos.y, startPos.z, MAP_ALL_LIQUIDS, NULL ) )
                {
                    ...
                    if ( farFromEndPoly )
                    {
                        if ( LIQUID_MAP_NO_WATER == baseMap->getLiquidStatus( endPos.x, endPos.y, endPos.z, MAP_ALL_LIQUIDS, NULL ) )
                        {
                            BuildShortcut();
                            _type = PATHFIND_NOPATH;
                            return;
                ...
                else if ( LIQUID_MAP_NO_WATER == baseMap->getLiquidStatus( endPos.x, endPos.y, endPos.z, MAP_ALL_LIQUIDS, NULL ) )
                {
                    if ( farFromEndPoly )
                    {
                        BuildShortcut();
    ...
    if ( _type == PATHFIND_NORMAL && cutToFirstHigher ) // starting in water, far from bottom, target is on the ground (above starting Z) -> update beginning points that are lower than starting Z
    {
        ...
        if ( i && i != size && LIQUID_MAP_NO_WATER != baseMap->getLiquidStatus( _pathPoints[ i - 1 ].x, _pathPoints[ i - 1 ].y, _pathPoints[ i - 1 ].z, MAP_ALL_LIQUIDS, NULL ) )
        {
