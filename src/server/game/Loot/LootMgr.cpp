...
bool LootItem::AllowedForPlayer(Player const* player) const
{
    ...
    if ((pProto->Flags2 & ITEM_FLAGS_EXTRA_HORDE_ONLY) && player->GetTeam(CrossFactionTeam::Discard) != TEAM_HORDE)
        return false;

    if ((pProto->Flags2 & ITEM_FLAGS_EXTRA_ALLIANCE_ONLY) && player->GetTeam(CrossFactionTeam::Discard) != TEAM_ALLIANCE)
        return false;
...
void Loot::FillLootFromDB(Creature *pCreature, Player* pLootOwner, uint32 mapId, uint8 mode)
{
    ...
    QueryResult result = CharacterDatabase.PQuery("SELECT itemId, itemCount, playerGuids FROM group_saved_loot WHERE creatureId = '%u' AND instanceId = '%u' AND mapId = '%u' AND difficultyId = '%u'", pCreature->GetEntry(), pCreature->GetInstanceId(), pCreature->GetMapId(), pCreature->GetMap()->GetDifficulty());
    ...
    if (result)
    {
        ...
        if (pMap && pMap->Instanceable() && WrathRaids(pMap->GetId()))
        {
            ...
            for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
            {
                if (Player* i_pl = i->GetSource())
                {
                    ...
                    {
                        Group::MemberSlotList gList = pGroup->GetMemberSlots();
                        for (Group::MemberSlotList::const_iterator itr = gList.begin(); itr != gList.end(); ++itr)
                            ss << itr->name << " (" << itr->guid << ")  ";
...
void Loot::removeItemFromSavedLoot(LootItem *item)
{
    ...
    Creature *pCreature = pMap->GetCreature(m_creatureGUID);

    if (!pCreature)
    {
        sLog->outLoot("Loot::removeItemFromSavedLoot: pCreature not found !! guid: %u, instanceid: %u) ", m_creatureGUID, pMap->GetInstanceId());
        return;
    }
    ...
    if (count > 1)
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_LOOT_SYSTEM_ITEM_COUNT_CRE);

        stmt->setUInt32(0, pCreature->GetInstanceId());
        stmt->setUInt32(1, item->itemid);
        stmt->setUInt32(2, pCreature->GetEntry());
        CharacterDatabase.Execute(stmt);
    }
    else
    {
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_LOOT_SYSTEM_ITEM);
...
void Loot::RemoveSavedLootFromDB(Creature * pCreature)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_LOOT_SYSTEM_CREATURE_LOOT);

    stmt->setUInt32(0, pCreature->GetEntry());
    stmt->setUInt32(1, pCreature->GetInstanceId());
    CharacterDatabase.Execute(stmt);
}
...
void Loot::saveLootToDB(Player *owner)
{
    ...
    if (!pCreature)
    {
        sLog->outLoot("Loot::saveLootToDB: pCreature not found !! Player: %s Guid: (%u)", owner->GetName().c_str(), owner->GetGUIDLow());
        return;
    }
    ...
    for (std::vector<LootItem>::iterator iter = items.begin(); iter != items.end(); ++iter)
    {
        ...
        if (proto->Quality >= ITEM_QUALITY_RARE)
        {
            ...
            if (count > 1)
            {
                PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_LOOT_SYSTEM_ITEM_COUNT);
                stmt->setUInt32(0, count);
                stmt->setUInt32(1, item->itemid);
                stmt->setUInt32(2, pCreature->GetInstanceId());
                CharacterDatabase.Execute(stmt);
            }
            else
            {
                PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_LOOT_SYSTEM_ITEM);
                stmt->setUInt32(0, pCreature->GetEntry());
                stmt->setUInt32(1, pCreature->GetInstanceId());
                stmt->setUInt32(2, pCreature->GetMapId());
                stmt->setUInt8(3, pCreature->GetMap()->GetDifficulty());
                stmt->setUInt32(4, item->itemid);
                stmt->setUInt32(5, count);
                stmt->setBool(6, pCreature->IsSummon());
                stmt->setFloat(7, pCreature->GetPositionX());
                stmt->setFloat(8, pCreature->GetPositionY());
                stmt->setFloat(9, pCreature->GetPositionZ());
                stmt->setString(10, guids.str());
                CharacterDatabase.Execute(stmt);
            }
            ss << "[" << item->itemid << "] ";
        }
...
void Loot::setItemLooted(LootItem *pLootItem, Player* looter)
{
    ...
    if (pLootItem->freeforall || !pLootItem->conditions.empty()) // those are not saved, trying to remove them will cause log spam
        return;
