...
bool Guild::BankTab::SetItem(SQLTransaction& trans, uint8 slotId, ItemRef const& item)
{
    if (slotId >= GUILD_BANK_MAX_SLOTS)
        return false;
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
    ...
    if (pInvitee->GetTeam() != player->GetTeam())
    {
...
{
    Player* player = session->GetPlayer();
    if (player->GetTeam() != sObjectMgr->GetPlayerTeamIdByGUID(GetLeaderGUID()))
        return;
...
{
    if (session->GetSecurity() && session->GetSecurity() < 10)
        return;
...
void Guild::_CreateDefaultGuildRanks(LocaleConstant loc, SQLTransaction& trans)
{
...
bool Guild::_CreateRank(std::string const& name, uint32 rights, SQLTransaction& trans)
{
