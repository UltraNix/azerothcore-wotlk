...
{
    PROFILE_SCOPE( "Map::HandleDelayedVisibility" );
...
bool Map::GetAreaInfo(uint32 phaseMask, float x, float y, float z, uint32& flags, int32& adtId, int32& rootId, int32& groupId) const
{
    ...
    bool hasDynamicAreaInfo = phaseMask && _dynamicTree.getAreaInfo(x, y, dynamic_z, phaseMask, dflags, dadtId, drootId, dgroupId);
...
void InstanceMap::SummonUnlootedCreatures()
{
    ...
    QueryResult result = CharacterDatabase.PQuery("SELECT DISTINCT creatureId, mapId, difficultyId, position_x, position_y, position_z FROM group_saved_loot WHERE instanceId = '%u' AND mapId = '%u' AND difficultyId = '%u' AND summoned = 1", GetInstanceId(), GetId(), GetDifficulty());
    ...
    if (result)
    {
        do
        {
            ...
            uint8 difficultyId = fields[2].GetUInt8();

            float pos_x = fields[3].GetFloat();
            float pos_y = fields[4].GetFloat();
            float pos_z = fields[5].GetFloat();

            TempSummon* pCreature = new TempSummon(0, 0, true);

            if (!pCreature->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), this, PHASEMASK_NORMAL, creatureId, 0, pos_x, pos_y, pos_z, 0))
            {
                delete pCreature;
                continue;
            }

            pCreature->SummonCreature(TEMPSUMMON_MANUAL_DESPAWN, 0);
            pCreature->loot.FillLootFromDB(pCreature, NULL, mapId, difficultyId);
...
void Map::LogEncounterFinished(EncounterCreditType type, uint32 creditEntry)
{
    ...
    CharacterDatabase.PExecute("INSERT INTO log_encounter (time, map, difficulty, creditType, creditEntry, playersInfo) VALUES(NOW(), %u, %u, %u, %u, '%s')", GetId(), (uint32)GetDifficulty(), type, creditEntry, playersInfo.c_str());
}
