...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_ACCOUNT_BANNED_BY_USERNAME, "SELECT account.id, username FROM account, account_banned WHERE account.id = account_banned.id AND active = 1 AND username LIKE CONCAT('%%', ?, '%%') GROUP BY account.id, username", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_UPD_LOGONPROOF, "UPDATE account SET sessionkey = ?, last_ip = ?, last_local_ip = ?, last_login = NOW(), locale = ?, failed_logins = 0, os = ? WHERE username = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_SEL_PINFO, "SELECT aa.gmlevel, a.username, a.email, a.last_ip, DATE_FORMAT(a.last_login, '%Y-%m-%d %T'), a.mutetime, a.mutereason, a.muteby FROM account a LEFT JOIN account_access aa ON (a.id = aa.id AND (aa.RealmID = ? OR aa.RealmID = -1)) WHERE a.id = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_INS_INGAME_STATISTICS, "INSERT INTO ingame_stats (connectedPlayers, playersCount, diff, avgDiff, currentTime, RealmID) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_SEL_PREMIUM_SERVICES, "SELECT premium_type, set_date, unset_date FROM account_premium WHERE id = ? AND RealmID = ?", CONNECTION_SYNCH);
}
