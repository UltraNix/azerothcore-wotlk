...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_CHARACTER, "SELECT guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
                     ...
                     "health, power1, power2, power3, power4, power5, power6, power7, instance_id, speccount, activespec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, autoInvite, blizzlikeMode, pvpAnnounces "
    ...
    PrepareStatement(CHAR_INS_CHARACTER, "INSERT INTO characters (guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
                     ...
                     "power4, power5, power6, power7, latency, speccount, activespec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, autoInvite, blizzlikeMode, pvpAnnounces) VALUES "
                     "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHARACTER, "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,playerBytes=?,playerBytes2=?,playerFlags=?,"
                     ...
                     "equipmentCache=?,ammoId=?,knownTitles=?,actionBars=?,grantableLevels=?,autoInvite=?,blizzlikeMode=?,pvpAnnounces=?,online=? WHERE guid=?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_STATS, "INSERT INTO character_stats (guid, maxhealth, maxpower1, maxpower2, maxpower3, maxpower4, maxpower5, maxpower6, maxpower7, strength, agility, stamina, intellect, spirit, "
                     ...
                     "spellPower, resilience, achievementPoints, avgItemLevel, penArmor, penSpell, ratExpertise, ratHaste, regMana, spellHit, meleeHit, rangeHit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    // SAVE LOOT SYSTEM
    PrepareStatement(CHAR_UPD_LOOT_SYSTEM_ITEM_COUNT, "UPDATE group_saved_loot SET itemCount = ? WHERE itemId = ? AND instanceId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_LOOT_SYSTEM_ITEM_COUNT_CRE, "UPDATE group_saved_loot SET itemCount = ? WHERE instanceId = ? AND itemId = ? AND creatureId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_DATA_FOR_CACHE, "SELECT guid, account, name, gender, race, class, level FROM characters WHERE guid = ? AND deleteDate IS NULL", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_ALL_CHARACTERS_DATA_FOR_CACHE, "SELECT guid, account, name, gender, race, class, level FROM characters WHERE deleteDate IS NULL", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_ALL_MAIL_COUNTS_FOR_CACHE, "SELECT receiver, COUNT(receiver) FROM mail GROUP BY receiver", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_MAIL_COUNT_BY_RECEIVER, "SELECT COUNT(receiver) FROM mail WHERE receiver = ?", CONNECTION_SYNCH);
}
