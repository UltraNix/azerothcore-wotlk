...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        switch (action)
        {
            ...
            {
                if (creature->GetEntry() == 95000 && !player->HasItemCount(85001))
                    player->AddItem(85001, 1);
                else if (creature->GetEntry() == 95001 && player->HasItemCount(85001))
                    player->AddItem(85002, 1);
                else if (creature->GetEntry() == 95002 && player->HasItemCount(85002))
                    player->AddItem(85003, 1);
                else if (creature->GetEntry() == 95003 && player->HasItemCount(85003))
                    player->AddItem(85004, 1);
                else if (creature->GetEntry() == 95004 && player->HasItemCount(85004))
                    player->AddItem(85005, 1);
            }
            break;
            case GOSSIP_ACTION_INFO_DEF + 2:
            {
                if (player->HasItemCount(85001) && player->GetPositionZ() > 325.0f && player->GetPositionZ() < 330.0f)
                    player->TeleportTo(0, 4313.801f, -2858.031f, 4.283f, player->GetOrientation());
            }
            break;
            case GOSSIP_ACTION_INFO_DEF + 3:
            {
                if (player->HasItemCount(85002) && player->GetPositionZ() > 145.0f && player->GetPositionZ() < 155.0f)
                    player->TeleportTo(1, 5440.668f, -2815.511f, 1464.347f, player->GetOrientation());
            }
            break;
            case GOSSIP_ACTION_INFO_DEF + 4:
            {
                if (player->HasItemCount(85003) && player->GetPositionZ() > 1570.0f && player->GetPositionZ() < 1580.0f)
                    player->TeleportTo(0, 3468.920f, -2790.761f, 87.631f, player->GetOrientation());
            }
            break;
            case GOSSIP_ACTION_INFO_DEF + 5:
            {
                if (player->HasItemCount(85004) && player->GetPositionZ() > 200.0f && player->GetPositionZ() < 210.0f)
                    player->TeleportTo(1, 5464.369f, -3512.380f, 1557.335f, player->GetOrientation());
            }
            break;
            case GOSSIP_ACTION_INFO_DEF + 6:
            {
                if (player->HasItemCount(85004) && player->GetPositionZ() > 1645.0f && player->GetPositionZ() < 1655.0f)
                    player->TeleportTo(37, -39.892f, -172.452f, 369.233f, player->GetOrientation());
            }
            break;
            case GOSSIP_ACTION_INFO_DEF + 7:
            {
                if (player->HasItemCount(85005) && player->GetPositionZ() > 375.0f && player->GetPositionZ() < 385.0f)
                {
                    sLog->outRewards("Player: %s GUID: %u has complete event 'Schody do nieba'", player->GetName().c_str(), player->GetGUIDLow());

                    for (uint8 i = 0; i < 6; i++)
                        if (player->HasItemCount(85001 + i, 1, false))
                            player->DestroyItemCount(85001 + i, 1, true);

                    {
                        PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_LORE_EVENT);
                        stmt->setUInt8(0, 2);
                        PreparedQueryResult result = WorldDatabase.Query(stmt);

                        if (!result)
                        {
                            player->TeleportTo(571, 5804.14f, 624.77f, 647.767f, 1.64f);
                            break;
                        }

                        uint8 eventComplete = (*result)[0].GetBool();
                        ...
                        {
                            std::string s = "Schody Do Nieba";
                            sWorld->SendWorldText(LANG_EVENT_WINNER, player->GetName().c_str(), s.c_str());
                            stmt = WorldDatabase.GetPreparedStatement(WORLD_UPD_LORE_EVENT);
                            stmt->setUInt8(0, 2);
...
enum Pytania
{
    ...
    PYTANIE_27             = 27000,
    PYTANIE_28             = 28000,
    PYTANIE_CORRECT_ANSWER = 30000,
    PYTANIE_WRONG_ANSWER   = 40000,

    NPC_GREETINGS_TEXT_1  = 950001,
    NPC_GREETINGS_TEXT_2  = 950002,
    NPC_GREETINGS_TEXT_3  = 950003,
    NPC_GREETINGS_TEXT_4  = 950004,
    NPC_GREETINGS_TEXT_5  = 950005,
    NPC_GREETINGS_TEXT_6  = 950006,
    NPC_GREETINGS_TEXT_7  = 950007,
    NPC_GREETINGS_TEXT_8  = 950008,
    NPC_GREETINGS_TEXT_9  = 950009,
    NPC_GREETINGS_TEXT_10 = 950010,
    NPC_GREETINGS_TEXT_11 = 950011,
    NPC_GREETINGS_TEXT_12 = 950012,
    NPC_GREETINGS_TEXT_13 = 950013,
    NPC_GREETINGS_TEXT_14 = 950014,
    NPC_GREETINGS_TEXT_15 = 950015,
    NPC_GREETINGS_TEXT_16 = 950016,

    NPC_CORRECT_TEXT_1    = 960001,
    NPC_CORRECT_TEXT_2    = 960002,
    NPC_CORRECT_TEXT_3    = 960003,
    NPC_CORRECT_TEXT_4    = 960004,
    NPC_CORRECT_TEXT_5    = 960005,
    NPC_CORRECT_TEXT_6    = 960006,
    NPC_CORRECT_TEXT_7    = 960007,
    NPC_CORRECT_TEXT_8    = 960008,
    NPC_CORRECT_TEXT_9    = 960009,
    NPC_CORRECT_TEXT_10   = 960010,
    NPC_CORRECT_TEXT_11   = 960011,
    NPC_CORRECT_TEXT_12   = 960012,
    NPC_CORRECT_TEXT_13   = 960013,
    NPC_CORRECT_TEXT_14   = 960014,
    NPC_CORRECT_TEXT_15   = 960015,
    NPC_CORRECT_TEXT_16   = 960016,
    NPC_CORRECT_TEXT_17   = 960017,
};
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Gotowy.", GOSSIP_SENDER_MAIN, PYTANIE_1);
            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_1, creature->GetGUID());
        }

        if (creature->GetEntry() == 96001 && player->HasItemCount(85101))
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Oczywiscie.", GOSSIP_SENDER_MAIN, PYTANIE_2);
            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_2, creature->GetGUID());
        }

        if (creature->GetEntry() == 96002 && player->HasItemCount(85102))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Jakie to pytanie?", GOSSIP_SENDER_MAIN, PYTANIE_3);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Jakie to pytanie?", GOSSIP_SENDER_MAIN, PYTANIE_4);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_3, creature->GetGUID());
        }

        if (creature->GetEntry() == 96003 && player->HasItemCount(85103))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Przeczytaj mi je.", GOSSIP_SENDER_MAIN, PYTANIE_5);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Przeczytaj mi je.", GOSSIP_SENDER_MAIN, PYTANIE_6);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_4, creature->GetGUID());
        }

        if (creature->GetEntry() == 96004 && player->HasItemCount(85104))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Zawsze jestem. Zaczynajmy.", GOSSIP_SENDER_MAIN, PYTANIE_7);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Zawsze jestem. Zaczynajmy.", GOSSIP_SENDER_MAIN, PYTANIE_8);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_5, creature->GetGUID());
        }

        if (creature->GetEntry() == 96005 && player->HasItemCount(85105))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "<Przyjrzyj sie zapisanym rycinom>", GOSSIP_SENDER_MAIN, PYTANIE_9);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "<Przyjrzyj sie zapisanym rycinom>", GOSSIP_SENDER_MAIN, PYTANIE_10);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_6, creature->GetGUID());
        }

        if (creature->GetEntry() == 96006  && player->HasItemCount(85106))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Chetnie poslucham nad czym debatujecie.", GOSSIP_SENDER_MAIN, PYTANIE_11);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Chetnie poslucham nad czym debatujecie.", GOSSIP_SENDER_MAIN, PYTANIE_12);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_7, creature->GetGUID());
        }

        if (creature->GetEntry() == 96007  && player->HasItemCount(85107))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Jaka to zagadka?", GOSSIP_SENDER_MAIN, PYTANIE_13);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Jaka to zagadka?", GOSSIP_SENDER_MAIN, PYTANIE_14);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_8, creature->GetGUID());
        }

        if (creature->GetEntry() == 96008  && player->HasItemCount(85108))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "<Wez zwoj z rak maga>", GOSSIP_SENDER_MAIN, PYTANIE_15);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "<Wez zwoj z rak maga>", GOSSIP_SENDER_MAIN, PYTANIE_16);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_9, creature->GetGUID());
        }

        if (creature->GetEntry() == 96009 && player->HasItemCount(85109))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "<Wez holoprojektor>", GOSSIP_SENDER_MAIN, PYTANIE_17);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "<Wez holoprojektor>", GOSSIP_SENDER_MAIN, PYTANIE_18);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_10, creature->GetGUID());
        }

        if (creature->GetEntry() == 96010  && player->HasItemCount(85110))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Oczywiscie, czytaj.", GOSSIP_SENDER_MAIN, PYTANIE_19);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Oczywiscie, czytaj.", GOSSIP_SENDER_MAIN, PYTANIE_20);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_11, creature->GetGUID());
        }

        if (creature->GetEntry() == 96011  && player->HasItemCount(85111))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Slucham wiec.", GOSSIP_SENDER_MAIN, PYTANIE_21);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Slucham wiec.", GOSSIP_SENDER_MAIN, PYTANIE_22);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_12, creature->GetGUID());
        }

        if (creature->GetEntry() == 96012  && player->HasItemCount(85112))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Jasne!", GOSSIP_SENDER_MAIN, PYTANIE_23);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Jasne!", GOSSIP_SENDER_MAIN, PYTANIE_24);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_13, creature->GetGUID());
        }

        if (creature->GetEntry() == 96013 && player->HasItemCount(85113))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Tak, jestem gotow.", GOSSIP_SENDER_MAIN, PYTANIE_25);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Tak, jestem gotow.", GOSSIP_SENDER_MAIN, PYTANIE_26);

            player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_14, creature->GetGUID());
        }

        if (creature->GetEntry() == 96014 && player->HasItemCount(85114))
        {
            if (rand == 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Gotow!", GOSSIP_SENDER_MAIN, PYTANIE_27);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Gotow!", GOSSIP_SENDER_MAIN, PYTANIE_28);

            if (player->GetTeamId() == TEAM_ALLIANCE)
                player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_15, creature->GetGUID());
            else
                player->SEND_GOSSIP_MENU(NPC_GREETINGS_TEXT_16, creature->GetGUID());
        }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        switch (action)
        {
            case PYTANIE_CORRECT_ANSWER:
            { 
                switch (player->GetSelectedAuction())
                {
                    case PYTANIE_1:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_1, creature->GetGUID());
                        if (!player->HasItemCount(85101))
                            player->AddItem(85101, 1);
                        break;
                    case PYTANIE_2:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_2, creature->GetGUID());
                        if (!player->HasItemCount(85102))
                            player->AddItem(85102, 1);
                        break;
                    case PYTANIE_3:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_3, creature->GetGUID());
                        if (!player->HasItemCount(85103))
                            player->AddItem(85103, 1);
                        break;
                    case PYTANIE_4:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_4, creature->GetGUID());
                        if (!player->HasItemCount(85104))
                            player->AddItem(85104, 1);
                        break;
                    case PYTANIE_5:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_5, creature->GetGUID());
                        if (!player->HasItemCount(85105))
                            player->AddItem(85105, 1);
                        break;
                    case PYTANIE_6:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_6, creature->GetGUID());
                        if (!player->HasItemCount(85106))
                            player->AddItem(85106, 1);
                        break;
                    case PYTANIE_7:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_7, creature->GetGUID());
                        if (!player->HasItemCount(85107))
                            player->AddItem(85107, 1);
                        break;
                    case PYTANIE_8:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_8, creature->GetGUID());
                        if (!player->HasItemCount(85108))
                            player->AddItem(85108, 1);
                        break;
                    case PYTANIE_9:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_9, creature->GetGUID());
                        if (!player->HasItemCount(85109))
                            player->AddItem(85109, 1);
                        break;
                    case PYTANIE_10:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_10, creature->GetGUID());
                        if (!player->HasItemCount(85110))
                            player->AddItem(85110, 1);
                        break;
                    case PYTANIE_11:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_11, creature->GetGUID());
                        if (!player->HasItemCount(85111))
                            player->AddItem(85111, 1);
                        break;
                    case PYTANIE_12:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_12, creature->GetGUID());
                        if (!player->HasItemCount(85112))
                            player->AddItem(85112, 1);
                        break;
                    case PYTANIE_13:
                        player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_13, creature->GetGUID());
                        if (!player->HasItemCount(85113))
                            player->AddItem(85113, 1);
                        break;
                    case PYTANIE_14:
                        if (player->GetTeamId() == TEAM_ALLIANCE)
                            player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_14, creature->GetGUID());
                        else
                            player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_15, creature->GetGUID());

                        if (!player->HasItemCount(85114))
                            player->AddItem(85114, 1);

                        break;
                    case PYTANIE_15:
                    {
                        if (player->GetTeamId() == TEAM_HORDE)
                            player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_16, creature->GetGUID());
                        else
                            player->SEND_GOSSIP_MENU(NPC_CORRECT_TEXT_17, creature->GetGUID());


                        for (uint8 i = 0; i < 15; i++)
                            if (player->HasItemCount(85101 + i, 1, false))
                                player->DestroyItemCount(85101 + i, 1, true);
            ...
            case PYTANIE_WRONG_ANSWER:
            {
                ...
                {
                    case PYTANIE_1:
                      message = "Tak myslalem, chciales mnie oszukac. Wroc jak namyslisz sie, na uczciwa pomoc.";
                      break;
                    case PYTANIE_2:
                      message = "Co za niedorzecznosc! Drwisz ze mnie? Naucze Cie szacunku do magow Kirin Toru!";
                      break;
                    case PYTANIE_3:
                      message = "Co za bzdury! Chcesz, bym sie zkompromitowal przed ta pieknoscia? Pokaze Ci, czemu nie zadziera sie z magami!";
                      break;
                    case PYTANIE_4:
                      message = "Nie kupuje tego, jakas totalna glupota. Wroc do miasta i poczytaj pare ksieg, wtedy wrocisz pewniejszy w swoje slowa.";
                      break;
                    case PYTANIE_5:
                      message = "Nie, nie, nie, zupelnie nie tak, wroc gdy bedziesz znac odpowiedz.";
                      break;
                    case PYTANIE_6:
                      message = "Magiczna plapka, uwazaj!";
                      break;
                    case PYTANIE_7:
                      message = "Nie, nie, nie, to sie nie zgadza. Psujesz nam tylko humor. Nie chcemy tu takich kanciarzy.";
                      break;
                    case PYTANIE_8:
                      message = "Argghhh! Nie masz pojecia co mowisz! Zmykaj stad, nim piraci rzuca Cie rekinom na pozarcie!";
                      break;
                    case PYTANIE_9:
                      message = "Co za niedorzecznosc! Drwisz ze mnie? Naucze Cie szacunku do magow Kirin Toru!";
                      break;
                    case PYTANIE_10:
                      message = "Nie nie, uciekaj stad. Wroc, gdy sie ogrzejesz i przemyslisz odpowiedz.";
                      break;
                    case PYTANIE_11:
                      message = "Na pewno nie. Nic mi po Twojej pomocy. Precz!";
                      break;
                    case PYTANIE_12:
                      message = "Co za niedorzecznosc! Drwisz ze mnie? Naucze Cie szacunku do magow Kirin Toru!";
                      break;
                    case PYTANIE_13:
                      message = "Nieee, to nie to. Uciekaj napic sie browaru, ale w Dalaranie!";
                      break;
                    case PYTANIE_14:
                      message = "Niestety, nie pomozesz mi. No nic, wroc do mnie, gdy przypomnisz sobie odpowiedz na to pytanie.";
                      break;
                    default:
                      message = "Blad, zapraszam na poprawke!";
                      break;
                }
            ...
            case PYTANIE_1:
            {
                player->SetSelectedAuction(PYTANIE_1);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "1. Jak nazywal sie ostatni ksiaze wysokich elfow, pochodzacy z rodu Sunstrider?", GOSSIP_SENDER_MAIN, PYTANIE_1);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Kel'Thuzad.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Kael'Thas.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Kil'Jeaden.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Khaz'Rogal.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_2:
            {
                player->SetSelectedAuction(PYTANIE_2);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "2. W kim zakochani byli obaj bracia Stormrage.", GOSSIP_SENDER_MAIN, PYTANIE_2);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) W Maiev Shadowsong.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) W Sylvanas Windrunner.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) W Tyrande Whisperwind.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) W Krolowej Azsharze.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_3:
            {
                player->SetSelectedAuction(PYTANIE_3);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "3. W jakim Zywiole specjalizuje sie arcymistrzyni magii, Jaina Proudmoore?", GOSSIP_SENDER_MAIN, PYTANIE_3);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Ogniem.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Powietrzem.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Woda.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Ziemia.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_4:
            {
                player->SetSelectedAuction(PYTANIE_3);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "4. Jaki przydomek dostal Varian Wrynn podczas epizodu swego zycia jako gladiator?", GOSSIP_SENDER_MAIN, PYTANIE_4);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Sha'rel.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Do'rogh.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Ker'zul.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Lo'gosh.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_5:
            {
                player->SetSelectedAuction(PYTANIE_4);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "5. Co z jezyka orkow oznacza przydomek Variana Wrynna, Lo'gosh?", GOSSIP_SENDER_MAIN, PYTANIE_5);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Krew wrogow.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Sila przodkow.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Lwi krol.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Wilczy duch.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_6:
            {
                player->SetSelectedAuction(PYTANIE_4);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "6. Doradczyni krola Stormwind, Lady Katrana Prestor to tak naprawde:", GOSSIP_SENDER_MAIN, PYTANIE_6);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Alexstrasza.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Onyxia.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Ysera.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Chronormu (Chromie).", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_7:
            {
                player->SetSelectedAuction(PYTANIE_5);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "7. Jakie imie nosil Deathwing jako smoczy aspekt?", GOSSIP_SENDER_MAIN, PYTANIE_7);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Nozdormu.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Nathanos.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Neltharion.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Nefarian.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_8:
            {
                player->SetSelectedAuction(PYTANIE_5);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "8. Gdzie wychowywal sie Thrall zanim zostal wodzem hordy?", GOSSIP_SENDER_MAIN, PYTANIE_8);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) W Durnholde Keep.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) W Frostwolf Keep.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) W jaskiniach gor Alterac.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) W Orgrimmarze.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_9:
            {
                player->SetSelectedAuction(PYTANIE_6);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "9. Czyim synem jest Thrall?", GOSSIP_SENDER_MAIN, PYTANIE_9);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Durotara.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Durotana.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Orgrima.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Gul'dana.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_10:
            {
                player->SetSelectedAuction(PYTANIE_6);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "10. Orkowie, by zdobyc nadludzka sile, a w konsekwencji ulec mocy i stac sie poslusznymi, wypili krew jednego z demonow. Ktorego?", GOSSIP_SENDER_MAIN, PYTANIE_10);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Maghteridona.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Mannorotha.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Archimonda.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Aggonara.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_11:
            {
                player->SetSelectedAuction(PYTANIE_7);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "11. Za ktorym z wladcow smierci gonil w zemscie Arthas?", GOSSIP_SENDER_MAIN, PYTANIE_11);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Za Tichondriusem.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Za Mal'Ganisem.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Za Balnazzarem.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Za Varimathrasem.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_12:
            {
                player->SetSelectedAuction(PYTANIE_7);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "12. Ktory z wladcow smierci sluzyl Sylvanie Windrunner?", GOSSIP_SENDER_MAIN, PYTANIE_12);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Tichondrius.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Mal'Ganis.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Balnazzar.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Varimathras.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_13:
            {
                player->SetSelectedAuction(PYTANIE_8);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "13. Jak nazywa sie czempion krolowej banshee zyjacy w Eastern Plaguelands?", GOSSIP_SENDER_MAIN, PYTANIE_13);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Augustus the Touched.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Koltira Deathweaver.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Jorad Mace.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Nathanos Blightcaller.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_14:
            {
                player->SetSelectedAuction(PYTANIE_8);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "14. Jak nazywal sie kon Tiriona Fordringa?", GOSSIP_SENDER_MAIN, PYTANIE_14);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Barador.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Pinador.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Torador.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Mirador.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_15:
            {
                player->SetSelectedAuction(PYTANIE_9);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "15. Kim jest Rexxar?", GOSSIP_SENDER_MAIN, PYTANIE_15);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Pol orkiem, pol trollem.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Pol orkiem, pol draenei.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Pol orkiem, pol ogrem.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Pol ogrem, pol trollem.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_16:
            {
                player->SetSelectedAuction(PYTANIE_9);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "16. Ile siostr i braci miala Sylvanas?", GOSSIP_SENDER_MAIN, PYTANIE_16);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Tylko dwie siostry.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Dwie siostry i brata.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Dwie siostry i dwoch braci.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Siostre i dwoch braci.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_17:
            {
                player->SetSelectedAuction(PYTANIE_10);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "17. Dlaczego czesc nocnych elfow zostala wygnana i zmuszona zalozyc wlasne panstwo, stajac sie potem wysokimi elfami?", GOSSIP_SENDER_MAIN, PYTANIE_17);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Zostali opetani przez demony.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Wbrew zakazom potajemnie uzywali magii.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Nie uznali nowej wladzy.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Chcieli ukrasc swieta wode ze studni wiecznosci.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_18:
            {
                player->SetSelectedAuction(PYTANIE_10);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "18. Ktory ze smokow podjal sie opieki nad Anveena, awatarem studni slonca?", GOSSIP_SENDER_MAIN, PYTANIE_18);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Vaelastrasz.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Korialstrasz.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Alexstrasza.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Xerestrasza.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_19:
            {
                player->SetSelectedAuction(PYTANIE_11);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "19. Tirion Fordring byl szanowanym mieszkancem w Hearthglen, ale przez pewien incydent zostal z niego wygnany. Jaki?", GOSSIP_SENDER_MAIN, PYTANIE_19);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Spalil koszary w miescie.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Zabil burmistrza Hearthglen.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Pobil straz miasta w obronie orka.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Zerwal sztandar wojsk, obrazajacy jego pochodzenie.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_20:
            {
                player->SetSelectedAuction(PYTANIE_11);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "20. Czego brakuje przywodcy plemienia Amani, Zul'Jinowi?", GOSSIP_SENDER_MAIN, PYTANIE_20);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Wsparcia Duchow.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Wlasnego miejsca w Azeroth.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Reki i oka.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Piatej klepki.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_21:
            {
                player->SetSelectedAuction(PYTANIE_12);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "21. Dlaczego Akama sluzyl Illidanowi w Czarnej Swiatyni?", GOSSIP_SENDER_MAIN, PYTANIE_21);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Chcial byc potezniejszy, a Illidan mogl mu te moc dac.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Chcial wyzwolic swiatynie spod wladzy demonow, ale Illidan sam zajal swiatynie.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Chcial sie sprawdzic w bitwie, a Illidan mial wielu wrogow.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Zostal przekupiony, chciwosc wziela gore.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_22:
            {
                player->SetSelectedAuction(PYTANIE_12);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "22. Kim byl Sargeras zanim stal sie zly?", GOSSIP_SENDER_MAIN, PYTANIE_22);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Tak zwanym Old Godem.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Tytanem.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Aspektem zycia.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) On zawsze byl zly.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_23:
            {
                player->SetSelectedAuction(PYTANIE_13);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "23. Ktory z niebieskich smokow polegl broniac studni slonca przed demonami?", GOSSIP_SENDER_MAIN, PYTANIE_23);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Azuregos.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Cyanigosa.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Madrigosa.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Kalecgos.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_24:
            {
                player->SetSelectedAuction(PYTANIE_13);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "24. By zatrzymac plage czarnej krwi, jeden z druidow Cenarion Circle zabral sadzonki drzewa zycia i zasadzil je w calym Azeroth. Kto byl tym druidem?", GOSSIP_SENDER_MAIN, PYTANIE_24);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Malfurion Stormrage.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Ysiel Windsinger.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Fandral Staghelm.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Hamuul Runetotem.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_25:
            {
                player->SetSelectedAuction(PYTANIE_14);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "25. Ktory z zielonych smokow, wymienionych poni¿ej, nie jest opiekunem portalu do Emerald Dream?", GOSSIP_SENDER_MAIN, PYTANIE_25);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Merithra.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Ysondre.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Lethon.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Taerar.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_26:
            {
                player->SetSelectedAuction(PYTANIE_14);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "26. Ktory z nizej wymienionych nie jest starym bogiem?", GOSSIP_SENDER_MAIN, PYTANIE_26);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) N'Zoth.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Il'gynoth.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Y'Shaarj.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) C'Thun.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_27:
            {
                player->SetSelectedAuction(PYTANIE_15);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "27. Kto przed Arthasem zasiadal na tronie Lich Kinga?", GOSSIP_SENDER_MAIN, PYTANIE_27);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Drek'thar.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Ner'zhul.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Anub'arak.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Kel'Thuzad.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            case PYTANIE_28:
            {
                player->SetSelectedAuction(PYTANIE_15);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "28. W Wojnie Trzech Mlotow walczyly 3 klany krasnoludzkich wojownikow. Ktory klan nie bral udzialu w walkach?", GOSSIP_SENDER_MAIN, PYTANIE_28);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A) Wildhammer.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "B) Bronzebeard.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "C) Dark Iron.", GOSSIP_SENDER_MAIN, PYTANIE_WRONG_ANSWER);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "D) Frostborn.", GOSSIP_SENDER_MAIN, PYTANIE_CORRECT_ANSWER);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            };
        }
