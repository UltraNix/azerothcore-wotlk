        ...
        void UpdateAI(uint32 diff)
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_MINIBOSS_SPAWN_HELPERS:
                {
                    ...
                    {
                        if (cr = me->SummonCreature(NPC_TWILIGHT_EGG, EggsPos[isSartharion ? i+12 : i].GetPositionX(), EggsPos[isSartharion ? i+12 : i].GetPositionY(), EggsPos[isSartharion ? i+12 : i].GetPositionZ(), EggsPos[isSartharion ? i+12 : i].GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000))
                        {
                            summons.Summon(cr);
...
public:
    ...
    struct boss_sartharion_vesperonAI : public ScriptedAI
    {
        ...
        void ClearInstance()
        {
            ...
            {
                pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_TWILIGHT_SHIFT);
                pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_TWILIGHT_TORMENT_VESPERON_AURA);
