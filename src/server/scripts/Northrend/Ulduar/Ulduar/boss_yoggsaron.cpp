...
const float PortalSpawn25[10][4] =
{
        ...
        void AddPortals()
        {
            ...
            for (uint8 i = 0; i < RAID_MODE(4, 10); ++i)
            {
                ...
                if (cr = me->SummonCreature(NPC_DESCEND_INTO_MADNESS,
                    RAID_MODE(PortalSpawn10[i][0], PortalSpawn25[i][0]),
                    RAID_MODE(PortalSpawn10[i][1], PortalSpawn25[i][1]),
                    RAID_MODE(PortalSpawn10[i][2], PortalSpawn25[i][2]),
                    RAID_MODE(PortalSpawn10[i][3], PortalSpawn25[i][3]),
    ...
    public:
        ...
        class spell_yogg_saron_lunatic_gaze_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                for (std::list<WorldObject*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
                {
                    bool MindOrDrainCaster = false;

                    if ((*itr)->GetTypeId() == TYPEID_PLAYER)
                        if (Spell* curSpell = (*itr)->ToPlayer()->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
                            if (curSpell->m_spellInfo->Id == 47855 || curSpell->m_spellInfo->Id == 48156)
                                MindOrDrainCaster = true;

                   if ((*itr)->HasInArc(M_PI, GetCaster()) && !MindOrDrainCaster)
