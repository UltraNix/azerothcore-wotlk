            ...
            void UpdateAI(uint32 diff)
            {
                ...
                switch (events2.ExecuteEvent())
                {
                    case EVENT_KRIK_START_WAVE:
                    {
                        ...
                        int8 wavesInCombat = 0;
                        Creature* watchers[3];

                        watchers[0] = summons.GetCreatureWithEntry(NPC_WATCHER_NARJIL);
                        watchers[1] = summons.GetCreatureWithEntry(NPC_WATCHER_GASHRA);
                        watchers[2] = summons.GetCreatureWithEntry(NPC_WATCHER_SILTHIK);

                        for (int8 i = 2; i >= 0; i--)
                            if (watchers[i])
                                if (watchers[i]->IsInCombat())
                                    wavesInCombat++;

                        if(wavesInCombat < waveCount)
