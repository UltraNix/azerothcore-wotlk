...
class ouro_spawner : public CreatureScript
{
public:
    ouro_spawner() : CreatureScript("ouro_spawner") { }

    CreatureAI* GetAI(Creature* creature) const
    {
        return new ouro_spawnerAI(creature);
    }

    struct ouro_spawnerAI : public ScriptedAI
    {
        ouro_spawnerAI(Creature* creature) : ScriptedAI(creature)
        { 
            ouroSpawned = false;
        }
            ...
            if (who->GetTypeId() == TYPEID_PLAYER)
            {
                if (me->IsWithinDistInMap(who, 0.5f))
                {
                    if (!ouroSpawned)
                    {
                        ...
                        if (Creature* Ouro = me->SummonCreature(15517, x, y, z, o, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 7200000))
                            Ouro->AI()->AttackStart(who);
                    }
                }
            }
        }
