                    ...
                    if (PhaseTimer <= diff)
                    {
                        switch (Step)
                        {
                            ...
                            case 38:
                                ...
                                {
                                    Position pos;
                                    Varimathras->GetNearPosition(pos, 15.0f, static_cast<float>(M_PI));
...
class npc_varimathras_battle_undercity : public CreatureScript
{
    public:
        ...
        struct npc_varimathras_battle_undercityAI : public ScriptedAI
        {
            ...
            {
                if (!Portals.empty())
                    if (Creature* RandomPortal = Trinity::Containers::SelectRandomContainerElement(Portals))
            ...
            void SummonMobs()
            {
                ...
                if (!Portals.empty())
                    for (std::list<Creature*>::iterator itr = Portals.begin(); itr != Portals.end(); ++itr)
                        if (Creature* Portal = (*itr))
                        {
                            uint32 WaveEntry = 0;
                            switch (WaveCount)
                            {
