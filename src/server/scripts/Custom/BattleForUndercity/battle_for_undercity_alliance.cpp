...
class npc_varian_battle_undercity : public CreatureScript
{
    public:
                    ...
                    while (uint32 eventId = _events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            case EVENT_UPDATE_INTRO_WORLDSTATE:
                            {
                                ...
                                if (--EventRepeatCount)
                                    _events.ScheduleEvent(EVENT_UPDATE_INTRO_WORLDSTATE, 1 * MINUTE * IN_MILLISECONDS);
                                else
                                    _events.ScheduleEvent(EVENT_START_BATTLE, 1 * MINUTE * IN_MILLISECONDS);
                                break;
                            }
                            case EVENT_START_BATTLE:
                                ...
                                DoCast(me, SPELL_STRENGTH_OF_WRYNN, false);
                        ...
                        switch (Step)
                        {
                            ...
                            case 20:
                                ...
                                if (Creature* Putress = ObjectAccessor::GetCreature(*me, PutressGUID))
                                    if (Putress->GetAI())
                                    {
                                        Putress->GetAI()->SetGUID(PlayerGUID);
                                        Putress->GetAI()->SetData(0, 2);
                                    }
                                me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_NONE);
                                TalkEvent = false;
                                break;
...
class npc_putress_battle_undercity : public CreatureScript
{
    public:
        npc_putress_battle_undercity() : CreatureScript("npc_putress_battle_undercity") { }
                    ...
                    case 1:
                    {
                        for (uint8 i = 0; i < 5; ++i)
                            if (Creature* Tesla = me->SummonCreature(NPC_PUTRESS_TRIGGER, PutressTeslaTriggersPos[i]))
                            {
                                ...
                                Tesla->CastSpell(me, SPELL_BLIGHT_EMPOWEREMENT, false, NULL, NULL, me->GetGUID());
                            }
                        break;
                    }
                    case 2:
                    ...
                    if (PhaseTimer <= diff)
                    {
                        switch (Phase)
                        {
                            ...
                            case 5:
                                ...
                                DoCast(me, SPELL_PUTRESS_TESLA_AURA, true);
                                DoCast(SPELL_BLIGHT_OVERLOAD);
...
{
    new npc_varian_battle_undercity();
    new npc_putress_battle_undercity();
}
