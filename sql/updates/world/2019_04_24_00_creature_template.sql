...
UPDATE `creature_template` SET `health_mod` = @PROF_BASE + (@PROF_BASE * 0) WHERE `entry` = @PROF;

SET @VOLATILE_OOZE:= 38759;
SET @VOLATILE_OOZE_BASE:= 350;

UPDATE `creature_template` SET `health_mod` = @VOLATILE_OOZE_BASE + (@VOLATILE_OOZE_BASE * 0) WHERE `entry` = @VOLATILE_OOZE;

SET @GAS:= 38761;
SET @GAS_BASE:= 350;

UPDATE `creature_template` SET `health_mod` = @GAS_BASE + (@GAS_BASE * 0) WHERE `entry` = @GAS;
...
UPDATE `creature_template` SET `health_mod` = @BQL_BASE + (@BQL_BASE * 0) WHERE `entry` = @BQL;

SET @BLAZING_SKELETON:= 38722;
SET @BLAZING_SKELETON_BASE:= 25;

UPDATE `creature_template` SET `health_mod` = @BLAZING_SKELETON_BASE + (@BLAZING_SKELETON_BASE * 0) WHERE `entry` = @BLAZING_SKELETON;

SET @GLUT:= 38734;
SET @GLUT_BASE:= 94;

UPDATE `creature_template` SET `health_mod` = @GLUT_BASE + (@GLUT_BASE * 0) WHERE `entry` = @GLUT;

SET @BLISTERING:= 38733;
SET @BLISTERING_BASE:= 24;

UPDATE `creature_template` SET `health_mod` = @BLISTERING_BASE + (@BLISTERING_BASE * 0) WHERE `entry` = @BLISTERING;

SET @RISEN_ARCH:= 38735;
SET @RISEN_ARCH_BASE:= 30;

UPDATE `creature_template` SET `health_mod` = @RISEN_ARCH_BASE + (@RISEN_ARCH_BASE * 0) WHERE `entry` = @RISEN_ARCH;

SET @SINDRA:= 38267;
SET @SINDRA_BASE:= 3300;

UPDATE `creature_template` SET `health_mod` = @SINDRA_BASE + (@SINDRA_BASE * 0) WHERE `entry` = @SINDRA;

SET @LK:= 39168;
SET @LK_BASE:= 7397;

UPDATE `creature_template` SET `health_mod` = @LK_BASE + (@LK_BASE * 0) WHERE `entry` = @LK;

SET @GHOUL:= 39311;
SET @GHOUL_BASE:= 60;

UPDATE `creature_template` SET `health_mod` = @GHOUL_BASE + (@GHOUL_BASE * 0) WHERE `entry` = @GHOUL;

SET @VILE:= 39286;
SET @VILE_BASE:= 21.4286;

UPDATE `creature_template` SET `health_mod` = @VILE_BASE + (@VILE_BASE * 0) WHERE `entry` = @VILE;

SET @WICKED:= 39289;
SET @WICKED_BASE:= 0.634921;

UPDATE `creature_template` SET `health_mod` = @WICKED_BASE + (@WICKED_BASE * 0) WHERE `entry` = @WICKED;
