...
UPDATE `creature_template` SET `health_mod` = @STEELBREAKER_25_BASE + (@STEELBREAKER_25_BASE * 0.25) WHERE `entry` = @STEELBREAKER_25;
...
UPDATE `creature_template` SET `health_mod` = @COLLAPSING_STAR_25_BASE + (@COLLAPSING_STAR_25_BASE * 0.4) WHERE `entry` = @COLLAPSING_STAR_25;
